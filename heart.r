...
# Base R
sales_dataset_1000 <- read.csv("C:/Users/Admin/Downloads/sales_dataset_1000.csv", stringsAsFactors = FALSE)
# Or using readr (tidyverse)
library(readr)
sales_dataset_1000 <- read_csv("C:/Users/Admin/Downloads/sales_dataset_1000.csv")


head(sales_dataset_1000)



# Check structure and column types
str(sales_dataset_1000)
# View first few rows
head(sales_dataset_1000)
# Summary statistics for numeric columns
summary(sales_dataset_1000)



# Count missing values in each column
colSums(is.na(sales_dataset_1000))

sales_dataset_1000 <- sales_dataset_1000[ , !names(sales_dataset_1000) %in% c("UnwantedColumn1", "UnwantedColumn2")]

colnames(sales_dataset_1000) <- c("ID", "Product", "Sales", "Date")  # example

sales_dataset_1000$Date <- as.Date(sales_dataset_1000$Date, format="%Y-%m-%d")


sales_dataset_1000$Sales <- as.numeric(sales_dataset_1000$Sales)


# Total sales by product
aggregate(Sales ~ Product, data = sales_dataset_1000, sum)
# Basic plot
library(ggplot2)
ggplot(sales_dataset_1000, aes(x=Product, y=Sales)) + geom_bar(stat="identity")
